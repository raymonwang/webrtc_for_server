...
class WebRtcVideoChannel2 : public VideoMediaChannel, public webrtc::Transport {
 public:
  ...
  void SendKeyFrame(const uint32_t ssrc){
	rtc::CritScope stream_lock(&stream_crit_);
	const auto& kv = send_streams_.find(ssrc);
	if (kv != send_streams_.end())
	  kv->second->SendKeyFrame();
  }

  void RequestKeyFrame(const uint32_t ssrc){
	rtc::CritScope stream_lock(&stream_crit_);
	const auto& kv = receive_streams_.find(ssrc);
	if (kv != receive_streams_.end())
	  kv->second->RequestKeyFrame();
  }
