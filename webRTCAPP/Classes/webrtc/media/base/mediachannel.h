...
struct MediaConfig {
  ...
  struct Video {
    ...
    // (lfz): Modify this flag from false to true. A video server don't need to
    // render decoded frame, so don't need to startup incomingvideostream thread
    bool disable_prerenderer_smoothing = true;  //false;
