...
int NetEqImpl::GetAudio(AudioFrame* audio_frame, bool* muted) {
  ...
  rtc::CritScope lock(&crit_sect_);
  int error = GetAudioInternalV2(audio_frame, muted);
  if (error != 0) {
    error_code_ = error;
    return kFail;
  }
...
int NetEqImpl::GetAudioInternal(AudioFrame* audio_frame, bool* muted) {
  ...
  *muted = false;
  ...
  return return_value;
    ...
    int NetEqImpl::GetAudioInternalV2(AudioFrame* audio_frame, bool* muted)
    {
        PacketList packet_list;
        Operations operation = kNormal;
        *muted = false;
        ...
        assert(sync_buffer_.get());
//        int cache_num = (decoder_frame_length_ != 0 && num_output_samples_per_channel != 0) ? 240 / (decoder_frame_length_ * 10 / num_output_samples_per_channel) : 10;
        const int cache_num = 10;
        if (sync_buffer_->FutureLength() >= num_output_samples_per_channel) {
            sync_buffer_->GetNextAudioInterleaved(num_output_samples_per_channel, audio_frame);
            audio_frame->sample_rate_hz_ = fs_hz_;
        }
        else if (packet_buffer_->NumPacketsInBuffer() <= cache_num) {
//            playout_timestamp_ += static_cast<uint32_t>(output_size_samples_);
//            audio_frame->sample_rate_hz_ = fs_hz_;
//            audio_frame->samples_per_channel_ = output_size_samples_;
...
//            *muted = true;
            ...
            size_t required_samples = 2 * output_size_samples_;
            int extracted_samples = ExtractPackets(required_samples, &packet_list);
            ...
            DoNormal(decoded_buffer_.get(), length, speech_type, false);
...
//            << ")";
        ...
        uint32_t temp_timestamp = sync_buffer_->end_timestamp() - static_cast<uint32_t>(sync_buffer_->FutureLength());
        if (static_cast<int32_t>(temp_timestamp - playout_timestamp_) > 0) {
            playout_timestamp_ = temp_timestamp;
        }
        audio_frame->timestamp_ = first_packet_ ? 0 : timestamp_scaler_->ToExternal(playout_timestamp_) - static_cast<uint32_t>(audio_frame->samples_per_channel_);
