...
namespace webrtc {
    ...
    public:
        ...
        virtual int32_t PreProcessTransmitMix() override;
        // mixaudio for special channel
        virtual int32_t ProcessForChannel(const int32_t channelID, bool mix_all = false) override;
        virtual void ClearTransmitMixCache() override;
    ...
    private:
        ...
        void UpdateToMix(std::map<int, MixerParticipant*>& mixParticipantList, size_t& maxAudioFrameCounter);
        
        void UpdateTransmitMixedStatus(std::map<int, MixerParticipant*>& mixedParticipantsList);
        
        int PreProcessTransmitMixV1();
        
        int PreProcessTransmitMixV2();
        
        int GetAllMixAudioFrame();
        
        int32_t ProcessForChannelInnerV2(const int32_t channelID, bool mix_all);
        
        int32_t MixFromList(AudioFrame* mixed_frame, const AudioFrameList& audioFrameList, int mix_factor);
        
        void AddFrameToAvailableContainer(int64_t energy_level, AudioFrame* audio_frame);
        
        void GetBackFrameFromChannel(MixerParticipant* participant) const;
    ...
    private:
        ...
        AudioFrameList _mixList;
        AudioFrameList _rampOutList;
        AudioFrameList _additionalFramesList;
        webrtc::AudioFrame*      middle_frame_;
        std::multimap<uint64_t, AudioFrame*>     available_mix_frame_container_;
        std::unordered_map<int, std::set<int>>  channel_mix_history_;
//        std::unordered_map<int, float>      factor_container_;
    };
}  // namespace webrtc
