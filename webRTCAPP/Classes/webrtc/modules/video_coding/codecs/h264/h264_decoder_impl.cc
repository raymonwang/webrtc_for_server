...
int32_t H264DecoderImpl::InitDecode(const VideoCodec* codec_settings,
  ...
  parser.reset(av_parser_init(av_context_->codec_id));
...
int32_t H264DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  while (offset < input_image._length) {
    int len = av_parser_parse2(parser.get(),
    						   av_context_.get(),
							   &packet.data,
							   &packet.size,
							   input_image._buffer + offset,
							   input_image._length - offset,
							   packet.pts,
							   packet.dts,
							   packet.pos);
    offset += len;
    ...
    if (packet.size) {
      ...
      if (0 > result && AVERROR_EOF != result) {
	  	LOG(LS_ERROR) << "avcodec_send_packet failed, result is " << result;
		return WEBRTC_VIDEO_CODEC_ERROR;
	  }
        

      result = avcodec_receive_frame(av_context_.get(), av_frame_.get());
      if (0 > result && AVERROR(EAGAIN) != result) {
		LOG(LS_ERROR) << "avcodec_receive_frame failed, result is " << result;
		return WEBRTC_VIDEO_CODEC_ERROR;
	  }
