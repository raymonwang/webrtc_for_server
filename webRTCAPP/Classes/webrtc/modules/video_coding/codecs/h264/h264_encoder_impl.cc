...
int32_t H264EncoderImpl::InitEncode(const VideoCodec* codec_settings,
  ...
  key_frame_interval_ = max_frame_rate_ * 10;
  ...
  // (lfz): shutdown logging
  int level = WELS_LOG_QUIET;
  openh264_encoder_->SetOption(ENCODER_OPTION_TRACE_LEVEL, &level);
...
SEncParamExt H264EncoderImpl::CreateEncoderParams() const {
  ...
  encoder_params.iComplexityMode = HIGH_COMPLEXITY;
  ...
  encoder_params.sSpatialLayers[0].uiProfileIdc = PRO_SCALABLE_BASELINE;
  encoder_params.sSpatialLayers[0].uiLevelIdc = LEVEL_3_1;
  //encoder_params.sSpatialLayers[0].iDLayerQp = 37;
