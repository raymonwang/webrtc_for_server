...
namespace webrtc {
    ...
    namespace voe {
        ...
        struct SendBufferCache {
            SendBufferCache() {
                send_buffer_.reset(new uint8_t[4096]);
                send_buffer_len_ = 0;
                time_stamp_ = 0;
                payload_type_ = 0;
            }
            ...
            RTPFragmentationHeader rtp_header;
        ...
        public:
            ...
            int32_t SendCachedData(FrameType frameType,
                             const voe::SendBufferCache* buffer_cache);
            ...
            uint32_t EncodeAndSend(bool need_cache = false);
            ...
            int AddDecoodedFrame(AudioFrame* frame);
            AudioFrame* GetFrontFrame();
            AudioFrame* PopFrontFrame();
            int GetDecoodedFrameNum();
            int AddLowWaterMark();
            bool InLowWaterStatus();
            void ClearLowWaterStatus();
//            uint64_t PushEnergyToChannel(uint32_t energy);
            const SendBufferCache* GetSendBufferCache();
        ...
        private:
            ...
            std::queue<AudioFrame*> channel_frame_buffers_;
            std::queue<int> channel_energy_history_;
            int low_water_times_;
//            int64_t last_20_frames_energy_;
            bool need_cache_send_data_;
            SendBufferCache send_buffer_cache_;
            std::vector<uint32_t> csrcs_;
            uint64_t    temp_index_ = 0;
        };
