...
namespace webrtc {
    ...
    // sender audio conference process
    void VoEBaseImpl::SenderAudioMixerProcess()
    {
        voe::ChannelManager& channelManager = shared_->channel_manager();
        voe::ChannelManager::Iterator it(&channelManager);
        
        //从当前所有通道取出一个时间片的语音数据
        int can_mix_count = shared_->transmit_mixer()->PreProcessTransmitMix();
        if (can_mix_count == 0) {
            return;
        }
        
        for (int i = 0; i < can_mix_count; i++) {
            //先取出每个通道的当前的帧，并暂存在mcu中
            if (shared_->transmit_mixer()->MixForChannels(-1, true) == -1) {
                shared_->transmit_mixer()->ClearTransmitMixCache();
                continue;
            }
            //先处理公共通道编码并发送
            bool encoded = false;
            const voe::SendBufferCache* send_buffer_cache = nullptr;
            for (voe::ChannelManager::Iterator it(&channelManager); it.IsValid(); it.Increment()) {
                voe::Channel* channel = it.GetChannel();
                int32_t channelid = channel->ChannelId();
                if (!channel->Playing() && channel->Sending()) {
                    if (!encoded) {
                        shared_->transmit_mixer()->DemuxAndMix(channelid);
                        shared_->transmit_mixer()->EncodeAndSend(channelid, true);
                        send_buffer_cache = channel->GetSendBufferCache();
                        encoded = true;
                    }
                    else if (send_buffer_cache) {
                        channel->SendCachedData(kAudioFrameSpeech, send_buffer_cache);
                    }
                }
            }
            ...
            for (voe::ChannelManager::Iterator it(&channelManager); it.IsValid(); it.Increment()) {
                ...
                if (channel->Playing() && channel->Sending()) {
                    if (shared_->transmit_mixer()->MixForChannels(channelid) == 0) {
                        shared_->transmit_mixer()->DemuxAndMix(channelid);
                        shared_->transmit_mixer()->EncodeAndSend(channelid);
                    }
                }
            }
            shared_->transmit_mixer()->ClearTransmitMixCache();
    ...
    int32_t VoEBaseImpl::NeedMorePlayData()
    {
        ...
        audioFrame.sample_rate_hz_ = AudioConferenceMixer::kNbInHz;
        //从所有通道里收取10ms的数据存放到通道缓存里等待输出侧混音
        voe::ChannelManager& channelManager = shared_->channel_manager();
        voe::ChannelManager::Iterator it(&channelManager);
        
        for (voe::ChannelManager::Iterator it(&channelManager); it.IsValid();
             it.Increment()) {
            //暂定id为0
            it.GetChannel()->GetAudioFrameWithInfo(16000, &audioFrame);
        }
