...
namespace webrtc {
    namespace voe {
        ...
        int32_t Channel::SendData(FrameType frameType,
            ...
            if (temp_index_++ % 10 == 0) {
                csrcs_.clear();
                int count = _audioFrame.client_ssrcs_num > 3 ? 3 : _audioFrame.client_ssrcs_num;
                for (int i = 0; i < count; i++) {
                    csrcs_.push_back(_audioFrame.client_ssrcs[i*2]);
                    csrcs_.push_back(_audioFrame.client_ssrcs[i*2+1]);
                }
                if (csrcs_.size() > 0) {
                    _rtpRtcpModule->SetCsrcs(csrcs_);
                }
            }
            
            if (need_cache_send_data_) {
                memcpy(send_buffer_cache_.send_buffer_.get(), payloadData, payloadSize);
                send_buffer_cache_.send_buffer_len_ = payloadSize;
                if (fragmentation) {
                    send_buffer_cache_.rtp_header.CopyFrom(*fragmentation);
                }
                send_buffer_cache_.payload_type_ = payloadType;
                send_buffer_cache_.time_stamp_ = timeStamp;
                send_buffer_cache_.csrcs = csrcs_;
            }
        ...
        int32_t Channel::SendCachedData(FrameType frameType, const voe::SendBufferCache* buffer_cache)
        {
            if (!buffer_cache) {
                return -1;
            }
            ...
            if (temp_index_++ % 10 == 0) {
                if (buffer_cache->csrcs.size() > 0) {
                    _rtpRtcpModule->SetCsrcs(buffer_cache->csrcs);
                }
            }
            ...
            if (!_rtpRtcpModule->SendOutgoingData((FrameType&)frameType, buffer_cache->payload_type_, buffer_cache->time_stamp_, -1, buffer_cache->send_buffer_.get(), buffer_cache->send_buffer_len_, &buffer_cache->rtp_header, nullptr, nullptr)) {
                _engineStatisticsPtr->SetLastError(
                                                   VE_RTP_RTCP_MODULE_ERROR, kTraceWarning,
                                                   "Channel::SendData() failed to send data to RTP/RTCP module");
                return -1;
            ...
            _lastLocalTimeStamp = buffer_cache->time_stamp_;
            _lastPayloadType = buffer_cache->payload_type_;
        ...
        MixerParticipant::AudioFrameInfo Channel::GetAudioFrameWithMuted(
            ...
            GetRemoteSSRC(audioFrame->source_ssrc);
...
//            last_20_frames_energy_ = 0;
        ...
        int32_t Channel::StartPlayout() {
            ...
            if (!_externalMixing) {
                if (_transmitMixerPtr->SetMixabilityStatus(*this, true) != 0)
                {
                    LOG(LS_ERROR) << "StartSend() failed to add participant to mixer";
                    return -1;
                }
        ...
        int32_t Channel::StopPlayout() {
            ...
            if (!_externalMixing) {
                if (_transmitMixerPtr->SetMixabilityStatus(*this, false) != 0)
                {
                    LOG(LS_ERROR) << "StopSend() failed to add participant to mixer";
                    return -1;
                }
...
//                if (_transmitMixerPtr->SetMixabilityStatus(*this, true) != 0)
//                {
//                    _engineStatisticsPtr->SetLastError(VE_AUDIO_CONF_MIX_MODULE_ERROR, kTraceError,
//                                                       "StartSend() failed to add participant to mixer");
//                    return -1;
//                }
            }
...
//                {
//                    _engineStatisticsPtr->SetLastError(VE_AUDIO_CONF_MIX_MODULE_ERROR, kTraceError,
//                                                       "StopSend() failed to add participant to mixer");
//                    return -1;
//                }
            }
        ...
        uint32_t Channel::EncodeAndSend(bool need_cache) {
            WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, _channelId),
            ...
            _timeStamp += static_cast<uint32_t>(_audioFrame.samples_per_channel_);
            need_cache_send_data_ = need_cache;
        ...
        int Channel::AddDecoodedFrame(AudioFrame* frame)
        {
            channel_frame_buffers_.push(frame);
            return (int)channel_frame_buffers_.size();
        }
        
        AudioFrame* Channel::GetFrontFrame()
        {
            if (channel_frame_buffers_.size() == 0 || low_water_times_ > 0) {
                return nullptr;
            }
            AudioFrame* frame = channel_frame_buffers_.front();
            channel_frame_buffers_.pop();
            return frame;
        }
        
        AudioFrame* Channel::PopFrontFrame()
        {
            if (channel_frame_buffers_.size() == 0) {
                return nullptr;
            }
            AudioFrame* frame = channel_frame_buffers_.front();
            channel_frame_buffers_.pop();
        ...
        int Channel::GetDecoodedFrameNum()
        {
            return (int)channel_frame_buffers_.size();
        }
        
        int Channel::AddLowWaterMark()
        {
            return ++low_water_times_;
        }
        
        bool Channel::InLowWaterStatus()
        {
            return low_water_times_ > 0;
        }
        
        void Channel::ClearLowWaterStatus()
        {
            low_water_times_ = 0;
        }
        
//        uint64_t Channel::PushEnergyToChannel(uint32_t energy)
//        {
//            if (channel_energy_history_.size() >= 100) {
//                uint32_t top_energy = channel_energy_history_.front();
//                channel_energy_history_.push(energy);
//                channel_energy_history_.pop();
//                return last_20_frames_energy_ + energy - top_energy ;
//            }
//            else {
//                channel_energy_history_.push(energy);
//                return last_20_frames_energy_ += energy;
//            }
//        }
        
        const SendBufferCache* Channel::GetSendBufferCache()
        {
            return &send_buffer_cache_;
        }
