...
namespace webrtc {
    namespace voe {
        ...
        _mixerModule(*AudioConferenceMixer::Create(instanceId))
        {
            ...
            if (_mixerModule.RegisterMixedStreamCallback(this) == -1) {
                WEBRTC_TRACE(kTraceError, kTraceVoice, VoEId(_instanceId, -1),
                             "TransmitMixer::TransmitMixer() failed to register mixer"
                             "callbacks");
            }
        }
        ...
        int32_t TransmitMixer::SetMixabilityStatus(MixerParticipant& participant,
                                    bool mixable)
        {
            return _mixerModule.SetMixabilityStatus(&participant, mixable);
        }
        
        int32_t TransmitMixer::SetAnonymousMixabilityStatus(MixerParticipant& participant,
                                             bool mixable)
        {
            return _mixerModule.SetAnonymousMixabilityStatus(&participant, mixable);
        }
        
        int32_t TransmitMixer::PreProcessTransmitMix()
        {
            return _mixerModule.PreProcessTransmitMix();
        }
        
        //为单个通道在发送端执行一次混音
        int32_t TransmitMixer::MixForChannels(int channelid, bool mix_all)
        {
            return _mixerModule.ProcessForChannel(channelid, mix_all);
        }
        
        void TransmitMixer::ClearTransmitMixCache()
        {
            _mixerModule.ClearTransmitMixCache();
        }
        
        int32_t TransmitMixer::DemuxAndMix(int32_t channelid)
        {
            WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
                         "TransmitMixer::DemuxAndMix()");
            
            if (channelid != -1) {
                voe::ChannelOwner ch = _channelManagerPtr->GetChannel(channelid);
                voe::Channel* channel_ptr = ch.channel();
                if (channel_ptr && channel_ptr->Sending()) {
                    channel_ptr->Demultiplex(_audioFrame);
                    channel_ptr->PrepareEncodeAndSend(_audioFrame.sample_rate_hz_);
        ...
        int32_t TransmitMixer::EncodeAndSend(int32_t channelid, bool need_cache)
        {
            WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
                         "TransmitMixer::EncodeAndSend()");
            ...
            if (channelid != -1) {
                ...
                voe::Channel* channel_ptr = ch.channel();
                if (channel_ptr && channel_ptr->Sending()) {
                    channel_ptr->EncodeAndSend(need_cache);
                }
                return 0;
            }
                 ...
                 it.Increment())
            {
                ...
                if (channelPtr->Sending())
                {
                    channelPtr->EncodeAndSend(need_cache);
                }
            }
            return 0;
        }
        ...
        void TransmitMixer::NewMixedAudio(int32_t id,
