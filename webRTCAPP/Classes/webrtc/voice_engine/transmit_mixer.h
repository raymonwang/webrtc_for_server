...
namespace webrtc {
    ...
    namespace voe {
        ...
        public:
            ...
            int32_t PreProcessTransmitMix();
            
            //为单个通道在发送端执行一次混音
            int32_t MixForChannels(int channelid, bool mix_all = false);
            
            void ClearTransmitMixCache();
            
            int32_t DemuxAndMix(int32_t channelid = -1);
            
            int32_t EncodeAndSend(int32_t channelid = -1, bool need_cache = false);
