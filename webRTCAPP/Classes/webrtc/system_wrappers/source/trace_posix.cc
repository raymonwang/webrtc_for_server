...
int32_t TracePosix::AddTime(char* trace_message, const TraceLevel level) const {
  ...
  sprintf(trace_message, "(%4u:%2u:%2u %2u:%2u:%2u:%3u |%5lu) ", system_time->tm_year + 1900,
          system_time->tm_mon, system_time->tm_mday, system_time->tm_hour,
          system_time->tm_min, system_time->tm_sec, ms_time,
          static_cast<unsigned long>(dw_delta_time));
  // Messages are 33 characters.
  return 33;
}
